...
class DeletedTasksActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val root = findViewById<androidx.constraintlayout.widget.ConstraintLayout>(R.id.deletedTasksRoot)
        val toolbar = findViewById<MaterialToolbar>(R.id.deletedTasksToolbar)
        ...
        recyclerView = findViewById(R.id.deletedTasksRecyclerView)
        emptyView = findViewById(R.id.deletedTasksEmptyView)
        loading = findViewById(R.id.deletedTasksLoading)
    ...
    private fun loadDeletedTasks() {
        ...
        networkExecutor.execute {
            when (val result = SupabaseTasksApi.listTasks(token, TaskListFilter.DELETED)) {
                is SupabaseTasksApi.ListResult.Success -> runOnUiThread {
                    loading.visibility = View.GONE
                    adapter.submitList(result.tasks)
                    if (result.tasks.isEmpty()) {
                        emptyView.text = getString(R.string.tasks_deleted_empty)
                        emptyView.visibility = View.VISIBLE
                        recyclerView.visibility = View.GONE
                    } else {
                        emptyView.visibility = View.GONE
                ...
                is SupabaseTasksApi.ListResult.Failure -> runOnUiThread {
                    ...
                    recyclerView.visibility = View.GONE
                    adapter.submitList(emptyList())
                    emptyView.text = getString(R.string.tasks_deleted_load_failed) + "\n" + result.message
                    emptyView.visibility = View.VISIBLE
                    Toast.makeText(
                        this,
                        getString(R.string.tasks_deleted_load_failed) + "\n" + result.message,
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
